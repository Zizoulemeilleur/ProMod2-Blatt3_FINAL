...
public class ClientConnectionThread extends Thread {
	...
	public ClientConnectionThread(Socket socket, Logger logger, String filename) {
		this.socket = socket;
		this.logger = logger;
	...
	public void run() {
		...
		try {
			final InputStreamReader inputStream = new InputStreamReader(socket.getInputStream());
			...
			// Nothing to do here; we closed the socket.
		} catch (final Exception e) {
			logger.log(socket, "Fehler in handle message" + e.getMessage());
			e.printStackTrace();
		}

		ServerGui.decrementClients();
		logger.log(socket, "<-- LocalAddress abgemeldet: " + socket.getLocalAddress());
		logger.log(socket, "<-- InetAddress abgemeldet: " + socket.getInetAddress());
	}

	private void handleMessage(String message, Socket socket, BufferedWriter ausgang)
		...
		case "3":
			...
			final List<Person> personen = (List<Person>) objectInputStream.readObject();
			// Cast (Zaubert das Object in eine Liste von Prs
			logger.log(socket, "Personen vom Client bekommen. Anzahl: " + personen.size());
		...
		case "4":
			logger.log(socket, "4");
			final List<Person> geladenePersonen = laden(socket);

			objectOutputStream.flush();
			objectOutputStream.writeObject(geladenePersonen);
			objectOutputStream.flush(); // flushd (wie beim stuff das gewritete
			// "durch"
			logger.log(socket, "Schicke Personen an den Client. Anzahl: " + geladenePersonen.size());
	...
	private final void save(List<Person> personen, Socket socket) {
		...
		try {
			...
			for (final Person p : personen) {
				...
				datei.write(p.getName() + ";" + p.getVname() + ";" + p.getAnrede() + ";" + p.getStrasse() + ";"
						+ p.getPlz() + ";" + p.getOrt() + ";" + p.getTelefon() + ";" + p.getFax() + ";" + p.getBem());
	...
	private final List<Person> laden(Socket socket) {
		...
		final List<Person> daten = new ArrayList<Person>();
		...
		try {
			...
			final FileReader fr = new FileReader("adreli.csv");
			final BufferedReader br = new BufferedReader(fr);

			String zeile = null;
			while ((zeile = br.readLine()) != null) {
				liste = zeile.split(";");
				final Person p = new Person(liste[0], liste[1], liste[2], liste[3], liste[4], liste[5], liste[6],
						liste[7], liste[8]);
				daten.add(p);
			}
